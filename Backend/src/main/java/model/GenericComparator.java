...
public class GenericComparator {
  ...
  public GenericComparator(int totalGaps, int morningMinutes, int daysOfWeek, int noMealTimes) {
    if (Math.abs(totalGaps) > 100
        || Math.abs(morningMinutes) > 100
        || Math.abs(noMealTimes) > 100
        || Math.abs(daysOfWeek) > 100) {
    ...
    this.wrongProfessors = 0;
    this.missingCommitments = 0;
  }

  public GenericComparator(int totalGaps, int morningMinutes, int daysOfWeek, int noMealTimes,
      int wrongProfessors, int missingCommitments) {
    if (Math.abs(totalGaps) > 100
        || Math.abs(morningMinutes) > 100
        || Math.abs(noMealTimes) > 100
        || Math.abs(daysOfWeek) > 100
        || Math.abs(wrongProfessors) > 100
        || Math.abs(missingCommitments) > 100) {
      throw new IllegalArgumentException("Weights must be between -100 and 100, inclusive");
    }
    this.totalGaps = totalGaps;
    this.morningMinutes = morningMinutes;
